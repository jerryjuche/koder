          ...
          >
            <div className="flex items-center gap-3">
              <span className={cn(
                ...
                submitted && idx === metadata.correct_index && "border-green-500 bg-green-500 text-white",
                submitted && idx === selected && idx !== metadata.correct_index && "border-red-500 bg-red-500 text-white",
              )}>
                {String.fromCharCode(65 + idx)}
              </span>
              <span className="flex-1">{option}</span>
              {submitted && idx === metadata.correct_index && (
                <CheckCircle2 className="h-4 w-4 text-green-500 shrink-0" />
              )}
              {submitted && idx === selected && idx !== metadata.correct_index && (
          ...
          <div className={cn(
            ...
            <div>
              ...
              {metadata.explanation && (
                <p className="text-sm mt-1 opacity-80">{metadata.explanation}</p>
              )}
            </div>
          </div>
          {!isCorrect && (
