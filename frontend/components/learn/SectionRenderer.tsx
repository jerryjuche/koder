...
export default function SectionRenderer({
  ...
  const badgeStyle =
    sectionTypeBadges[sectionType] ||
    "bg-muted text-muted-foreground border-muted";

  const renderMarkdown = (content: string) => {
    const processed = preprocessCallouts(content);
    return (
      <div className="prose prose-sm max-w-none dark:prose-invert prose-headings:text-foreground prose-a:text-primary prose-code:before:content-none prose-code:after:content-none [&_p]:mb-3 [&_p:empty]:hidden [&_br]:block [&_br]:content-[''] [&_br]:mt-2">
        <ReactMarkdown
            ...
            div({ className, children, ...props }) {
              if (className?.startsWith("custom-callout")) {
                ...
                const styles: Record<string, string> = {
                  "callout-tip":
                    "bg-amber-50 dark:bg-amber-950/30 border-l-4 border-amber-400 text-amber-900 dark:text-amber-200",
                  "callout-example":
                    "bg-blue-50 dark:bg-blue-950/30 border-l-4 border-blue-400 text-blue-900 dark:text-blue-200",
                  "callout-warning":
                    "bg-red-50 dark:bg-red-950/30 border-l-4 border-red-400 text-red-900 dark:text-red-200",
                  "callout-info":
                    "bg-sky-50 dark:bg-sky-950/30 border-l-4 border-sky-400 text-sky-900 dark:text-sky-200",
                };
                ...
                return (
                  <div
                    className={`p-4 rounded-lg my-4 text-sm ${styles[calloutType] || ""}`}
                    {...props}
                  >
  ...
  const renderSectionHeader = () => (
    <div className="flex items-center gap-2 mb-3">
      <span
        className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium border ${badgeStyle}`}
      >
        {Icon}
        {label}
      </span>
  ...
  switch (sectionType) {
    ...
    case "assessment": {
      const multiFileMeta = (
        section.metadata as unknown as Record<string, unknown>
      )?.multiFile as MultiFileSpec | undefined;
      return (
        <div>
          {renderSectionHeader()}
          {section.title && (
          ...
          {section.content && (
            <div className="mb-6">{renderMarkdown(section.content)}</div>
          )}
          <SectionExercise
            problemReferences={problemReferences}
            language={language}
            multiFile={multiFileMeta}
          />
        </div>
      );
    }
