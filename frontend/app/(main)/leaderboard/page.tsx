    ...
    <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-500">
      <div className="text-center space-y-4 pt-4">
        ...
        <p className="text-brand-offwhite-muted">
          {leaderboard.length} students competing · Updated live
        </p>
      </div>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isFirst
                  ? "border-brand-muted-gold/50 w-72 h-44 shadow-brand-muted-gold/10"
                  : "border-brand-charcoal-border w-64 h-36",
              )}
            >
              <div
                className={cn(
                  "absolute -top-6 w-14 h-14 rounded-full flex items-center justify-center font-bold text-lg shadow-inner text-white border-4 border-brand-charcoal-base",
                  getUserColor(user?.colorIndex || 0),
                )}
              >
                {initials}
              </div>
              <div className="mt-8 text-center">
                <div
                  className={cn(
                    "font-bold",
                    isFirst
                      ? "text-xl text-brand-offwhite"
                      : "text-lg text-brand-offwhite-muted",
                  )}
                >
                  {user?.name || "Unknown"}
                </div>
      ...
      {user && (
        <div className="bg-gradient-to-r from-brand-charcoal-base to-brand-charcoal-card border border-brand-muted-gold/30 rounded-2xl p-5 flex items-center justify-between shadow-lg shadow-brand-muted-gold/5">
          <div className="flex items-center gap-5">
            ...
            <div
              className={cn(
                "w-12 h-12 rounded-full flex items-center justify-center font-bold text-xl text-white shadow-inner",
            ...
            <div>
              <div className="text-sm font-medium text-brand-muted-gold">
                Your Ranking
              </div>
              <div className="text-xl font-bold text-brand-offwhite font-sans">
                {user.name}
              </div>
            </div>
          </div>
          <div className="flex items-center gap-8 text-right">
            <div>
              <div className="text-xl font-bold text-brand-muted-gold">
                {user.xp.toLocaleString()}
              </div>
              <div className="text-xs text-brand-offwhite-muted font-medium uppercase tracking-wider">
                XP
              </div>
            </div>
            <div>
              <div className="text-xl font-bold text-brand-offwhite">
                {user.solvedCount}
              </div>
              <div className="text-xs text-brand-offwhite-muted font-medium uppercase tracking-wider">
                Solved
            ...
            <div>
              <div className="text-xl font-bold text-[#8DB4B9]">
                {(
                  leaderboard.find((e) => e.user.id === user.id)?.bestTimeMs ||
                  0
                ).toFixed(1)}
                ms
              </div>
        ...
        <div className="p-4 border-b border-brand-charcoal-border flex justify-between items-center bg-brand-charcoal-hover/30">
          <div className="relative w-64">
            <input
              ...
              className="w-full bg-brand-charcoal-base border border-brand-charcoal-border rounded-lg pl-10 pr-4 py-2 text-sm focus:outline-none focus:border-brand-muted-gold text-brand-offwhite"
            />
            <Search
          ...
          <div className="bg-brand-charcoal-base border border-brand-charcoal-border rounded-lg p-1 flex">
            <button className="px-4 py-1.5 text-sm font-medium text-brand-offwhite-muted hover:text-brand-offwhite">
              Weekly
            </button>
            <button className="px-4 py-1.5 text-sm font-medium text-brand-offwhite-muted hover:text-brand-offwhite">
              Monthly
            </button>
            <button className="px-4 py-1.5 text-sm font-medium bg-brand-charcoal-hover text-brand-offwhite rounded shadow-sm border border-brand-charcoal-border">
              All Time
            </button>
          </div>
        </div>
              ...
              return (
                <tr
                  ...
                  className={cn(
                    ...
                    isMe
                      ? "bg-brand-charcoal-hover border-l-2 border-l-brand-muted-gold"
                      : "hover:bg-brand-charcoal-hover/50",
                  )}
                >
                  <td className="px-6 py-4">
                    <div
                      className={cn(
                        "w-8 h-8 mx-auto rounded flex items-center justify-center font-mono text-sm font-bold",
                        rank === 1
                          ? "bg-brand-muted-gold/20 text-brand-muted-gold"
                          : rank === 2
                            ? "bg-brand-offwhite-muted/20 text-brand-offwhite"
                            : rank === 3
                  ...
                  <td className="px-2 py-4 text-center">
                    {entry.rankDelta === 0 ? (
                      <Minus
                        size={16}
                        className="text-brand-offwhite-muted mx-auto"
                      />
                    ) : entry.rankDelta > 0 ? (
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-4">
                      <div
                        className={cn(
                          "w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-inner",
                      ...
                      <div>
                        <div className="font-bold text-brand-offwhite flex items-center gap-2">
                          ...
                          {isMe && (
                            <span className="bg-brand-muted-gold/10 text-brand-muted-gold text-[10px] px-1.5 py-0.5 rounded border border-brand-muted-gold/30 uppercase tracking-wider">
                              You
                            </span>
                          )}
                        </div>
                  ...
                  <td className="px-6 py-4 text-right font-mono text-[#8DB4B9]">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      {(entry.bestTimeMs || 0).toFixed(1)}ms
                    </div>
                  </td>
                </tr>
              );
