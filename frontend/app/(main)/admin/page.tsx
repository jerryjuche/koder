...
export default function AdminDashboard() {
  ...
  const [ingestUrl, setIngestUrl] = useState(
    "https://github.com/cs3100/go-assignments",
  );
  ...
  const [courseModules, setCourseModules] = useState<
    Record<string, CurriculumModule[]>
  >({});
  const [togglingCourseModule, setTogglingCourseModule] = useState<
    string | null
  >(null);
  const [loadingCourses, setLoadingCourses] = useState(false);
  const [editingModuleName, setEditingModuleName] = useState<string | null>(
    null,
  );
  const [editingModuleValue, setEditingModuleValue] = useState("");

  const loadData = useCallback(async () => {
    const [statsRes, usageRes, logsRes, problemsRes, modulesRes, coursesRes] =
      await Promise.all([
        fetchAdminStats(),
        fetchAIUsageStats(),
        fetchAdminActivity(),
    ...
    if (coursesRes.success && coursesRes.data) {
      ...
      const modulePromises = coursesRes.data.map((c: Course) =>
        fetchModules(c.id),
      );
  ...
  useWebSocket(
    {
      ...
      "feedback.submitted": useCallback(() => {
        loadData();
      }, [loadData]),
    },
    [loadData],
  ...
  const handleIngest = async () => {
    ...
    if (successCount > 0) {
      toast.success(
        `Successfully ingested ${successCount} folder(s) or repository(s)!`,
      );
      loadData();
    }
  ...
  const handleToggleVisibility = async (problem: Problem) => {
    ...
    if (res.success) {
      ...
      toast.error(
        detail
          ? `${res.error?.message}: ${detail}`
          : res.error?.message || "Toggle failed",
      );
    }
  };
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500 pt-4 pb-12">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-brand-offwhite-muted">
            Manage problems, content ingestion, and platform operations
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <Link
          ...
          className="bg-brand-charcoal-card border border-amber-600/30 hover:border-amber-500/60 rounded-2xl p-6 block transition-all duration-200 hover:-translate-y-0.5 group"
        >
          ...
          <div className="text-lg font-bold text-brand-offwhite mb-0.5 group-hover:text-amber-300 transition-colors">
            Module Manager
          </div>
          <div className="text-xs text-brand-offwhite-muted/70">
            Lock, unlock, and manage modules
          </div>
        </Link>
        <div className="bg-brand-charcoal-card border border-brand-charcoal-border rounded-2xl p-6">
          ...
          <div className="text-3xl font-bold text-brand-offwhite mb-1">
        ...
        <div className="bg-brand-charcoal-card border border-brand-cool-accent/30 rounded-2xl p-6">
          ...
          <div className="text-3xl font-bold text-brand-offwhite mb-1">
            {stats?.total_ai_calls || 0}
          </div>
          <div className="text-sm text-brand-offwhite-muted font-medium">
            AI Calls
          </div>
          <div className="text-xs text-brand-offwhite-muted/70 mt-1">
            {stats?.ai_calls_today ?? 0} today
          </div>
        </div>
      </div>
        ...
        <div className="bg-brand-charcoal-card border border-brand-cool-accent/20 rounded-2xl p-5">
          <div className="flex items-center gap-2 text-sm text-brand-offwhite-muted mb-3">
            ...
            <span className="text-xs">
              {aiUsageStats.total_ai_calls} total calls
            </span>
          </div>
      ...
      <div className="bg-brand-charcoal-card border border-brand-charcoal-border rounded-2xl overflow-hidden">
        <div className="p-4 border-b border-brand-charcoal-border flex justify-between items-center bg-brand-charcoal-panel/50 sticky top-0 z-10">
          <div className="font-bold flex items-center gap-2 text-brand-offwhite">
            <FileText size={18} className="text-brand-offwhite" /> Problem
            Catalog{" "}
            ...
            <tbody className="divide-y divide-brand-charcoal-border/50">
              {filteredProblems.map((p, i) => {
                ...
                return (
                  <tr
                    key={i}
                    className="hover:bg-brand-charcoal-hover/50 transition-colors"
                  >
                    ...
                    <td className="px-6 py-4 text-brand-offwhite-muted">
                      {p.module}
                    </td>
                    <td className={cn("px-6 py-4 font-medium", diffColor)}>
                      {diffLabel}
                    </td>
                    <td className="px-6 py-4">
                      <span
                    ...
                    <td className="px-6 py-4">
                      <div
                        ...
                        className={cn(
                          "w-10 h-5 rounded-full relative cursor-pointer opacity-80 hover:opacity-100 transition-opacity",
                          isActive
                            ? "bg-brand-success"
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        ...
                        className="text-brand-offwhite-muted hover:text-brand-muted-gold transition-colors"
                        title="Edit problem"
                      >
                        <Pencil size={16} />
                      </button>
              ...
              {problemsError && (
                <tr>
                  <td colSpan={6} className="px-6 py-8 text-center">
                    <div className="text-brand-error mb-1">
                      Failed to load problems
                    </div>
                    <div className="text-brand-offwhite-muted text-xs">
                      {problemsError}
                    </div>
                  </td>
                </tr>
              ...
              {!problemsError && filteredProblems.length === 0 && (
                <tr>
                  <td
                    colSpan={6}
                    className="px-6 py-8 text-center text-brand-offwhite-muted"
                  >
                    No problems found.
                  </td>
                </tr>
      ...
      {/* ===== ACCORDION: Locks + Settings ===== */}
      <Accordion type="multiple" className="space-y-4">
        {/* Problem Module Locks */}
        {allModules.length > 0 && (
          <AccordionItem
            value="problem-locks"
            className="border border-brand-charcoal-border rounded-xl bg-brand-charcoal-card overflow-hidden"
          >
              ...
              <div className="flex items-center justify-between w-full gap-4">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="text-left min-w-0">
                    ...
                    <p className="text-[11px] text-brand-offwhite-muted/60">
                      Lock entire problem categories from student access
                    </p>
                  </div>
                </div>
                <Badge
                  variant="outline"
                  className="text-xs shrink-0 bg-brand-charcoal-base border-brand-charcoal-border text-brand-offwhite-muted/70"
                >
                  <Lock size={10} className="mr-1 text-amber-400/70" />
                  {allModules.filter((m) => m.is_locked).length}/
                  {allModules.length} locked
                </Badge>
              </div>
            </AccordionTrigger>
            <AccordionContent className="px-5 pb-5 pt-4">
              <Tabs defaultValue="go">
                <TabsList className="mb-4">
                  <TabsTrigger
                    value="go"
                    className="text-xs data-[state=active]:bg-cyan-500/15 data-[state=active]:text-cyan-400"
                  >
                    <span className="w-4 h-4 rounded text-[9px] font-bold bg-cyan-500/20 text-cyan-400 flex items-center justify-center mr-1.5">
                      G
                    </span>
                    Go
                  </TabsTrigger>
                  <TabsTrigger
                    value="python"
                    className="text-xs data-[state=active]:bg-amber-500/15 data-[state=active]:text-amber-400"
                  >
                    <span className="w-4 h-4 rounded text-[9px] font-bold bg-amber-500/20 text-amber-400 flex items-center justify-center mr-1.5">
                      P
                    </span>
                    Python
                  </TabsTrigger>
                ...
                {(["go", "python"] as const).map((lang) => {
                  const mods = allModules
                    .filter((m) =>
                      lang === "go"
                        ? !m.module_name.startsWith("python-")
                        : m.module_name.startsWith("python-"),
                    )
                  ...
                  return (
                    <TabsContent key={lang} value={lang} className="mt-0">
                      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 max-h-[420px] overflow-y-auto scrollbar-thin pr-1">
                        {mods.map((mod) => {
                          ...
                          return (
                            <div
                              key={mod.module_name}
                              className="group/card bg-brand-charcoal-base/80 border border-brand-charcoal-border rounded-xl p-3.5 transition-all hover:border-brand-charcoal-hover hover:-translate-y-0.5 hover:shadow-sm hover:shadow-black/20"
                            >
                              <div className="flex items-start justify-between mb-2">
                                <div className="min-w-0 flex-1">
                                  ...
                                  <span className="text-[11px] text-brand-offwhite-muted/40 font-mono block truncate">
                                    {mod.module_name}
                                  </span>
                                </div>
                                {isLocked && (
                              ...
                              <div className="flex items-center justify-between">
                                <span className="text-[11px] text-brand-offwhite-muted/50 font-medium">
                                  {mod.problem_count} problem
                                  {mod.problem_count !== 1 ? "s" : ""}
                                </span>
                                <div className="flex items-center gap-1">
                                  <Button
                                    ...
                                    size="sm"
                                    disabled={
                                      togglingModule === mod.module_name
                                    }
                                    onClick={async () => {
                                      setTogglingModule(mod.module_name);
                                      const res = await toggleProblemModuleLock(
                                        mod.module_name,
                                      );
                                      if (res.success) {
                                        setAllModules((prev) =>
                                          prev.map((m) =>
                                            m.module_name === mod.module_name
                                  ...
                                  {mod.problem_count === 0 && (
                                    <Button
                                      ...
                                      size="sm"
                                      disabled={
                                        deletingModule === mod.module_name
                                      }
                                      onClick={async () => {
                                        if (
                                          !confirm(
                                            `Permanently delete module "${displayName}"?`,
                                          )
                                        )
                                          return;
                                        setDeletingModule(mod.module_name);
                                        const res = await deleteProblemModule(
                                          mod.module_name,
                                        );
                                        if (res.success) {
                                          toast.success(
        ...
        {/* Curriculum Module Locks */}
        {courses.length > 0 && !loadingCourses && (
          <AccordionItem
            value="curriculum-locks"
            className="border border-brand-charcoal-border rounded-xl bg-brand-charcoal-card overflow-hidden"
          >
            <AccordionTrigger className="px-5 py-4 hover:bg-brand-charcoal-hover/30 transition-colors duration-200 [&[data-state=open]>svg]:!rotate-180">
              <div className="flex items-center justify-between w-full gap-4">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="text-left min-w-0">
                    ...
                    <p className="text-[11px] text-brand-offwhite-muted/60">
                      Lock curriculum modules from student access
                    </p>
                  </div>
                </div>
                <Badge
                ...
                >
                  ...
                  {courses.reduce(
                    (acc, c) =>
                      acc +
                      (courseModules[c.id] || []).filter((m) => m.locked)
                        .length,
                    0,
                  )}
                  /
                  {courses.reduce(
                    (acc, c) => acc + (courseModules[c.id] || []).length,
                    0,
                  )}{" "}
                  locked
                </Badge>
                ...
                {courses.map((course) => {
                  ...
                  return (
                    <AccordionItem
                      key={course.id}
                      value={course.id}
                      className="border border-brand-charcoal-border rounded-lg bg-brand-charcoal-base/40 overflow-hidden"
                    >
                      <AccordionTrigger className="px-3.5 py-2.5 hover:bg-brand-charcoal-hover/30 transition-colors text-sm [&[data-state=open]>svg]:!rotate-180">
                        <div className="flex items-center justify-between w-full gap-3">
                          <span className="text-brand-offwhite font-medium truncate">
                            {course.title}
                          </span>
                          <Badge
                            variant="outline"
                            className={cn(
                              "text-[10px] px-2 py-0 h-5 font-normal shrink-0",
                              lockedCount > 0
                      ...
                      <AccordionContent className="px-3.5 pb-3 pt-2">
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                if (res.success) {
                                  ...
                                  toast.error(
                                    res.error?.message || "Failed to toggle",
                                  );
                                }
        ...
        {allModules.length > 0 && (
          ...
          >
            <AccordionTrigger className="px-5 py-4 hover:bg-brand-charcoal-hover/30 transition-colors duration-200 [&[data-state=open]>svg]:!rotate-180">
              <div className="flex items-center justify-between w-full gap-4">
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="w-8 h-8 rounded-lg bg-brand-muted-gold/15 flex items-center justify-center shrink-0">
                  ...
                  <div className="text-left min-w-0">
                    ...
                    <p className="text-[11px] text-brand-offwhite-muted/60">
                      Rename modules and pin them to appear first
                    </p>
                  </div>
                </div>
                <Badge
                    ...
                    <TabsContent key={lang} value={lang} className="mt-0">
                      <div className="space-y-2 max-h-[400px] overflow-y-auto scrollbar-thin pr-1">
                        {mods.map((mod) => {
                          ...
                          const isEditing =
                            editingModuleName === mod.module_name;
                          return (
                            <div
                              key={mod.module_name}
                              className="group/row bg-brand-charcoal-base/60 border border-brand-charcoal-border rounded-xl px-4 py-3 transition-all hover:border-brand-charcoal-hover hover:bg-brand-charcoal-base/80"
                            >
                              <div className="flex items-center gap-3">
                                <div className="flex-1 min-w-0">
                                    ...
                                    {isEditing ? (
                                      <div className="flex items-center gap-1.5">
                                        <Input
                                          ...
                                          onBlur={async () => {
                                            const val =
                                              editingModuleValue.trim();
                                            if (
                                              val &&
                                              val !== currentDisplayName
                                            ) {
                                              const res =
                                                await upsertModuleMeta(
                                                  mod.module_name,
                                                  val,
                                                );
                                              if (res.success) {
                                  ...
                                  <span className="text-[11px] text-brand-offwhite-muted/40 font-mono block">
                                    {mod.module_name} · {mod.problem_count}{" "}
                                    problem{mod.problem_count !== 1 ? "s" : ""}
                                  </span>
                                </div>
                                  ...
                                  <Button
                                    ...
                                    onClick={async () => {
                                      ...
                                      if (res.success) {
                                        ...
                                        toast.success(
                                          next
                                            ? `"${currentDisplayName}" pinned`
                                            : `"${currentDisplayName}" unpinned`,
                                        );
                                      } else {
                                        toast.error(
                                          res.error?.message ||
                                    ...
                                    title={
                                      isPinned ? "Unpin module" : "Pin module"
                                    }
                                  >
                                    {isPinned ? (
                                      <Pin size={14} />
            ...
            <div className="flex items-center justify-between w-full gap-4">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="text-left min-w-0">
                  ...
                  <p className="text-[11px] text-brand-offwhite-muted/60">
                    Password reset delivery tracking (created → sent →
                    delivered)
                  </p>
                </div>
              </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-brand-charcoal-card border border-brand-charcoal-border hover:border-brand-muted-gold/30 transition-colors rounded-2xl p-6 relative overflow-hidden group">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 text-brand-offwhite font-bold mb-1">
                  <Github className="text-brand-offwhite" size={20} /> Ingest
                  GitHub Repo
                </div>
                <p className="text-sm text-brand-offwhite-muted mb-6">
                  Pull problems from repository
                </p>
                <textarea
                  value={ingestUrl}
            ...
            <div className="bg-brand-charcoal-card border border-brand-muted-gold/30 rounded-2xl p-6 relative overflow-hidden group">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 text-brand-offwhite font-bold mb-1">
                  <Wand2 className="text-brand-muted-gold" size={20} /> Enrich
                  All Problems
                </div>
                <p className="text-sm text-brand-offwhite-muted mb-6">
                  AI-generate hints, tags & difficulty
                </p>
                ...
                <div className="space-y-3 mb-6 font-mono text-xs">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-brand-muted-gold font-bold">
                      {draftsCount} drafts
                    </span>
                  </div>
                </div>
          ...
          <div className="bg-brand-charcoal-card border border-brand-charcoal-border rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-brand-charcoal-border flex items-center gap-2">
              <BugPlay size={18} className="text-brand-muted-gold" />
              <h3 className="font-bold text-brand-offwhite">Problem Reports</h3>
            </div>
            <div className="p-4">
          ...
          <div className="bg-brand-charcoal-card border border-brand-charcoal-border rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-brand-charcoal-border flex items-center gap-2">
              <ShieldCheck size={18} className="text-brand-muted-gold" />
              <h3 className="font-bold text-brand-offwhite">
                User Verification
              </h3>
            </div>
            <div className="p-4">
        ...
        <div className="lg:col-span-1 bg-brand-charcoal-card border border-brand-charcoal-border rounded-2xl overflow-hidden flex flex-col h-fit lg:sticky lg:top-6">
          <div className="p-5 border-b border-brand-charcoal-border flex justify-between items-center shrink-0">
            <h3 className="font-bold text-brand-offwhite flex items-center gap-2">
              <Activity size={18} className="text-brand-offwhite-muted" />{" "}
              Activity Log
            </h3>
            <div className="text-xs font-bold text-brand-success flex items-center gap-1.5">
          ...
          <div className="p-5 space-y-6 overflow-y-auto max-h-[calc(100vh-240px)] scrollbar-thin">
            {activityLogs.map((log, i) => {
              ...
              return (
                <div
                  key={log.id}
                  className="flex gap-4 relative animate-in slide-in-from-right-4 fade-in"
                >
                  ...
                  <p className="text-sm text-brand-offwhite-muted leading-tight pt-0.5">
                    {log.message}
                  </p>
                </div>
              );
            ...
            {activityLogs.length === 0 && (
              <div className="text-sm text-brand-offwhite-muted text-center pt-8">
                No recent activity
              </div>
            )}
          </div>
