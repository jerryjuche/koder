  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500 pt-4 pb-12">
      ...
      <Accordion type="multiple" className="space-y-4">
        ...
        {allModules.length > 0 && (
          <AccordionItem value="problem-locks" className="border border-brand-charcoal-border rounded-xl bg-brand-charcoal-card overflow-hidden">
            <AccordionTrigger className="px-5 py-4 hover:bg-brand-charcoal-hover/30 transition-colors duration-200 [&[data-state=open]>svg]:!rotate-180">
              <div className="flex items-center justify-between w-full gap-4">
                ...
                <Badge variant="outline" className="text-xs shrink-0 bg-brand-charcoal-base border-brand-charcoal-border text-brand-offwhite-muted/70">
                  <Lock size={10} className="mr-1 text-amber-400/70" />
                  {allModules.filter(m => m.is_locked).length}/{allModules.length} locked
                </Badge>
              </div>
            </AccordionTrigger>
        ...
        {courses.length > 0 && !loadingCourses && (
          <AccordionItem value="curriculum-locks" className="border border-brand-charcoal-border rounded-xl bg-brand-charcoal-card overflow-hidden">
            <AccordionTrigger className="px-5 py-4 hover:bg-brand-charcoal-hover/30 transition-colors duration-200 [&[data-state=open]>svg]:!rotate-180">
              <div className="flex items-center justify-between w-full gap-4">
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className="w-8 h-8 rounded-lg bg-amber-500/15 flex items-center justify-center shrink-0">
                ...
                <Badge variant="outline" className="text-xs shrink-0 bg-brand-charcoal-base border-brand-charcoal-border text-brand-offwhite-muted/70">
                  ...
                  {courses.reduce((acc, c) => acc + (courseModules[c.id] || []).length, 0)} locked
                </Badge>
              </div>
            </AccordionTrigger>
            <AccordionContent className="px-5 pb-5 pt-3">
              <Accordion type="multiple" className="space-y-2">
                {courses.map((course) => {
                  ...
                  return (
                    <AccordionItem key={course.id} value={course.id} className="border border-brand-charcoal-border rounded-lg bg-brand-charcoal-base/40 overflow-hidden">
                      <AccordionTrigger className="px-3.5 py-2.5 hover:bg-brand-charcoal-hover/30 transition-colors text-sm [&[data-state=open]>svg]:!rotate-180">
                        <div className="flex items-center justify-between w-full gap-3">
                          <span className="text-brand-offwhite font-medium truncate">{course.title}</span>
        ...
        {/* Module Settings */}
        {allModules.length > 0 && (
          <AccordionItem value="module-settings" className="border border-brand-charcoal-border rounded-xl bg-brand-charcoal-card overflow-hidden">
            <AccordionTrigger className="px-5 py-4 hover:bg-brand-charcoal-hover/30 transition-colors duration-200 [&[data-state=open]>svg]:!rotate-180">
              <div className="flex items-center justify-between w-full gap-4">
                <div className="flex items-center gap-2.5 min-w-0">
