...
-- Backfill username from student_id for existing users
UPDATE users SET username = student_id WHERE username IS NULL;

-- Backfill email from student_id where it looks like an email
UPDATE users SET email = student_id WHERE email IS NULL AND student_id LIKE '%@%';

-- Update get_full_profile function to include username and google_avatar_url
