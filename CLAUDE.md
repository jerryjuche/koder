...
| -------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `router.go`          | 335   | `App` struct, `NewRouter()` (~121 routes), `Shutdown()`                                                                                                                                                                                                                                                                                                                                     |
...
| `admin.go`           | 1,089 | `AdminHandler` — Ingest, Enrich, EnrichAll, AIAssist (8 actions), GetAdminStats, GetAIUsage, ListAllProblems, ToggleVisibility, UpdateProblem, PublishAllDrafts, Approve/Reject contributions, SearchUsers, ToggleUserVerified, ResetUserPassword, ListModuleMeta, UpsertModuleMeta, SetModulePin, ListAllModules, List/Toggle ProblemModuleLocks, DeleteProblemModule, SendProblemReminder |
...
### 6.3 Store Layer (`internal/store/` — 23 files, 6,890 LOC)

| File                 | Lines | Key Exports                                                                                                                                                                                                                                                                                                           |
| -------------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `store.go`           | 301   | `Store` interface (163 methods), `PostgresStore` struct, `NewPostgresStore` (MaxConns=10, MinConns=2, 30m lifetime, SimpleProtocol)                                                                                                                                                                                   |
...
| `submissions.go`     | 270   | CreateSubmission (90d TTL), GetProblemWithTestCases (JOIN), GetRecentSubmissionForProblem, LikeSubmission (ON CONFLICT DO NOTHING, returns whether inserted), GetSubmissionLikeSummary (author + problem context)                                                                                                       |
| `solution_explanations.go` | 126 | Cached AI code explanations: GetSolutionExplanation (cache read, nil on miss), UpsertSolutionExplanation (ON CONFLICT submission_id DO UPDATE), GetSolutionForExplain (passed + visible eligibility JOIN)                                                                                                             |
...
### 6.5 Email (`internal/email/` — 2 source + 2 test files, 452 + 248 LOC)
...
| File                | Lines | Key Exports                                                                                                                                                                                                                                         |
| ------------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `executor.go`       | 1,306 | `Executor` (semaphore=6), `Execute` (scoring), `ExecuteVisibleOnly` (test-only, caps first 3 visible cases), `formatGoLiteral` (recursive), `formatPythonLiteral` (null→None), `goToSnakeCase`, `EnhancePythonError`, `parseCompilerError` (3-pass) |
| `format.go`         | 53    | `FormatCode` (go/format.Source in-process / black via sandbox), `FormatSyntaxError`                                                                                                                                                                 |
| `parser.go`         | 132   | `ParseTestOutput` — 5 regex patterns + `parentSummaryRegex` (flushes GOT/WANT buffers on `--- FAIL: TestSolution`), state machine for GOT/WANT multi-line parsing                                                                                   |
...
| `sandbox_client.go` | 239   | `SandboxRequest/Response`, HTTP client (3 retries, exp backoff 2ⁿ×500ms, request timeout = `timeout_sec + requestTimeoutExtra`), `FormatFriendlySandboxError`                                                                                       |
...
## 7. Sandbox (`sandbox/` — 8 source + 2 test + Dockerfile + fly.toml, ~1,610 LOC, Zero External Dependencies)

| File                       | Lines | Purpose                                                                                                                                                                                                                         |
| -------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `main.go`                  | 400   | HTTP server on `:$PORT` — `/health`, `/version`, `/execute`, `/format`; language dispatcher; `classifyOutput` (4 regex patterns); `compileErrorMessage` (3-pass)                                                                |
| `format.go`                | 213   | `POST /format` — Python formatting via pinned black (`black -q -`, 30s timeout) → `{formatted, error}`; re-parses raw source with `ast.parse` for accurate line/col + class-specific tips; falls back to stripped black message |
...
| `format_test.go`           | 196   | 8 tests (6 black-gated): black formatting valid/syntax/extra-indent/empty/unsupported-language/quote-normalization + pure-unit friendly-message/prefix-strip                                                                    |
| `Dockerfile`               | 38    | 2-stage build based on `golang:1.26-alpine`, includes python3 + pinned `black==25.1.0`, bakes Go build cache via `/warmup` (cold first-compile ~23s → ~2s)                                                                      |
...
| `register/page.tsx`        | 291   | Client | Google-first or email/password, Go/Python language choice                                                    |
| `forgot-password/page.tsx` | 199   | Client | Email-only password reset (Resend): email input + "Check your inbox" state with resend button (60s cooldown) |
...
| `UserVerificationPanel.tsx` | 305   | Client | Debounced search (300ms), verified toggle, reset-password dialog                                                                                                                                       |
...
| `ui/AnimatedBackground.tsx` | 73    | Animated grid/hex background, rAF-driven, SSR-safe                  |
| `ui/ShapeGrid.tsx`          | 417   | SVG shape-grid hover interaction layer                              |
...
| `event.ts`               | 136   | `subscribe(type, callback)`, `useWebSocket(handlers, deps)`, 10 event types, auto-reconnect                                                                                                          |
| `UserContext.tsx`        | 108   | `UserProvider`, `useUser()`, `refreshUser()`, `setPrimaryLanguage()`, WebSocket XP auto-refresh                                                                                                      |
| `useNotifications.ts`    | 106   | `useNotifications()`, 15s/60s polling, markAsRead (optimistic), markAllAsRead, WS `solution.liked` refresh                                                                                               |
...
## 9. Database Migrations (`migrations/` — 55 files, ~27,544 LOC)
...
| 019 | `022_add_pin_hash.sql`                        | 2     | pin_hash on users (bcrypt) — **[OBSOLETE]** dropped by 050                                                           |
...
| 037 | `053_ai_solution_explanations.sql`           | 19    | Cached AI code explanations: submission_id PK → submissions CASCADE, JSONB techniques/strengths/improvements        |
...
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `users`         | `id UUID`               | username, email, password (bcrypt), role, xp, google_id, verified, username_set, color_index, primary_language                                                                     |
...
| POST   | `/auth/change-password`     | ChangePassword (current-password verify) | `change_password.go` |
...
| POST   | `/ai/explain`                         | Explain (deduped NIM) | `explain.go`       |
| POST   | `/ai/explain/chat`                    | ExplainChat           | `explain.go`       |
...
## 13. Real-Time Events (Broker + WebSocket — 10 Event Types)
...
| `solution.liked`                          | LikeSubmission handler              | useNotifications (author bell refresh) |
...
| `internal/api`      | `webhooks_test.go` (295 LOC)        | 13                                       |
...
- **Motivation:** the Best Practices showcase (Session 117) surfaced raw code but no teaching layer, and likers never reached the author. This session adds (1) a professional, AI-generated code explanation for every solution — structured summary/approach/Big-O complexity/key-techniques/strengths/improvements + a grounded follow-up chat, served via the existing NVIDIA NIM (DeepSeek V4 Flash) provider, (2) whole-row accordion cards so any click on a solution opens code + AI explain, and (3) instant notifications to a solution's author the first time each user likes it (DB row + WS `solution.liked` event; 7s poll fallback)
- **Backend — cache layer:** `migrations/053_ai_solution_explanations.sql` (new, 19 LOC) — `ai_solution_explanations` table (`submission_id` PK → submissions ON DELETE CASCADE, language, summary, approach, time/space_complexity, key_techniques/strengths/improvements JSONB, created_at) + `created_at DESC` index. One row per submission = the NIM call is billed at most once ever per unique solution
- **Backend — store:** `internal/store/solution_explanations.go` (new, 126 LOC) — `GetSolutionExplanation` (cache read, nil on miss), `UpsertSolutionExplanation` (ON CONFLICT (submission_id) DO UPDATE), `GetSolutionForExplain` (JOIN problems, eligibility = `status='passed'` AND `visible`), plus `submissions.go` — `LikeSubmission` now returns `(bool, error)` via `ct.RowsAffected() > 0` (fakes embed the nil `PostgresStore`, so only real call sites changed), new `GetSubmissionLikeSummary` (author + problem context for the notification). Store interface 159 → 163 methods
- **Backend — enricher:** `ExplainSolution`/`ExplainChat` (enricher.go 942 → 1,157 LOC) — strict-JSON `validateExplainResponse` (summary/approach/time/space required), prompt builders (`buildExplainSystemPrompt`/`buildExplainUserPrompt`/`buildExplainChatSystemPrompt`/`buildExplainChatUserPrompt`), chat grounded in the cached structured analysis; reuses `waitForRateLimit` (1s global gap shared with admin assist) + `cleanResponse`
- **Backend — API:** `internal/api/explain.go` (new, 220 LOC) — `ExplainHandler` with a scriptable `Explainer` interface (mirrors the `Formatter` pattern for testability); `POST /ai/explain` (`{submission_id}` → `{cached, explanation}` — cache hit skips the provider entirely, miss generates + upserts), `POST /ai/explain/chat` (`{submission_id, question}` → `{answer}`, ephemeral); auth + eligibility (404 `SOLUTION_NOT_FOUND` if not passed/visible), 502 `EXPLAIN_FAILED`/`EXPLAIN_CHAT_FAILED`, `LogAIUsage` per call (`explain_solution`/`explain_chat`). `community.go` (139 → 196 LOC) — `CommunityHandler` now takes the broker; `LikeSubmission` notifies the author (`solution_liked`, message embeds problem title, `related_id` = problem id) + publishes `solution.liked` with `{user_id, submission_id, problem_slug, problem_title}`; skips self-likes + duplicate likes (silent ON CONFLICT no-op)
- **Backend — router:** one shared `enricher.Enricher` created in `NewRouter` and passed to both `NewAdminHandler` + `NewExplainHandler` (the 1s NIM gap stays globally serialized; admin.go no longer constructs its own); `NewCommunityHandler(store, b)`; new `explainRateLimiter` (10 req/60s, NO bypass, `AIRateLimitMiddleware`), `POST /ai/explain` + `/ai/explain/chat` with 256KB body limit in the authenticated group; `App` gains `explainRateLimiter` (stopped in `Shutdown`) → ~121 routes
- **Frontend — lib:** `lib/api.ts` + `explainSolution()`/`explainSolutionChat()`; `lib/types.ts` + `SolutionExplanation`; `lib/event.ts` + `'solution.liked'`; `lib/useNotifications.ts` subscribes to `solution.liked` → invalidates the notifications cache + refetches instantly (poll remains the fallback)
- **Frontend — UI:** `components/best-practices/ExplainPanel.tsx` (new, 272 LOC) — progressive-disclosure AI panel below the code: summary, approach (rendered via the XSS-safe `renderMarkdown`), Time/Space complexity badges (O(n) notation), key-technique chips, strengths/improvements two-column lists, cached chip, follow-up chat (Enter to send, loading state). `SolutionCard.tsx` — whole-row accordion (`role="button"`, tabIndex, Enter/Space, `aria-expanded`, `cursor-pointer`, `select-none`, focus ring); the chevron is now a visual indicator; expanded body stopPropagation so code/explain clicks don't collapse; `parts.tsx` — `stopPropagation` on `LikeButton`/`ProblemLink`/`SolutionAuthorRow` so interactive children never toggle the card
- **Tests:** `internal/api/explain_test.go` (249 LOC, 9 tests: 401, payload validation, 404 ineligible, cache-hit skips provider, cache-miss upserts + usage log, provider 502, chat answer/validation/404), `internal/api/community_test.go` (140 LOC, 3 tests: new-like → notification + broker event, duplicate → silent, self-like → none — chi route context injected for `URLParam`), enricher_test.go 4 → 8 tests (ExplainSolution parse/fences/validation/error + ExplainChat + validateExplainResponse) → **195 backend tests** (was 179)
- **Verified:** `go vet ./internal/...` clean, `go build ./cmd/server` OK, 9/9 backend suites green (195 tests), `tsc --noEmit` 0 errors, ESLint 0 errors on all changed files
...
- **Noted:** the whole-row accordion covers all card variants (the top-3 podium described in Session 117 renders through `SolutionCard` — no separate `PodiumCard.tsx` exists)
...
### 2026-08-12 — Session 115: Password-reset email template light-theme polish

- **Refined** `internal/email/email.go` to finalize the light-theme transactional email shell for password reset and problem reminder messages: updated background/text palette values, tightened vertical spacing, improved CTA/button styling, and hardened the fallback URL block.
...
- **Updated** `internal/email/email_test.go` expectations to match the refreshed light-theme visuals and confirm safe HTML escaping for injected content.
- **Verified:** `go test ./internal/email/...` passes.
- **Docs:** `SESSION_LOG.md` and `UPDATE_LOG.txt` were appended with the new email template polish entry.
...
- **PIN system fully removed** — recovery is now email-only; change-password verifies the **current password** instead of a PIN:
  - Backend: `change_password.go` rewritten (266→148 LOC) — `SetPin`/`VerifyPin` deleted, `ChangePassword` takes `{current_password, new_password}`, verifies via bcrypt against `users.password` (5/15min brute-force limiter retained, `globalPinLimiter` → `globalPasswordLimiter`), 409 `NO_PASSWORD_SET` guard for Google-only accounts, 8–128 char + different-from-current validation
  - `pin_reset.go` (296 LOC) deleted entirely; `/auth/set-pin`, `/auth/verify-pin`, `/auth/forgot-password-pin`, `/auth/reset-password-pin` routes removed (router.go 303→298)
  - Store: `UpdateUserPINHash` removed (interface + impl), `pin_hash` dropped from all INSERTs/SELECTs (users.go 1,373→1,345), `PINHash` fields removed from `User`/`NewUser` (types.go 650→648, store.go 285→284)
  - Migration `050_drop_pin_hash.sql` (new, 7 LOC): `ALTER TABLE users DROP COLUMN IF EXISTS pin_hash;` — production Supabase must run this manually
- **Frontend**: forgot-password page (419→148) is now email-only (PIN tab removed); Settings Security tab change-password dialog (1,104→969) is a single current/new/confirm form with per-field **eye toggles** (`PasswordField` component) + "Forgot your current password? Reset via email" link; `lib/api.ts` (926→895) — `forgotPasswordPin`/`resetPasswordPin`/`verifyPin`/`setPin` deleted, `changePassword(currentPassword, newPassword)` posts `{current_password, new_password}`; `components/base/input/pin-input.tsx` (172) deleted (`input-otp` dep retained — shadcn `ui/input-otp.tsx` still uses it)
- **Docs**: README.md (API table, schema, file tree) + CLAUDE.md (§6.2 handler table, §12.1 auth routes, §10 users row, migration tables, §19 RESEND note) purged of PIN references; §17 metrics and file counts updated
- **Verified:** `go vet` clean, `go build ./cmd/server ./internal/...` OK, 8/8 backend suites green, `tsc --noEmit` 0 errors, ESLint 0 errors; counts: backend 63 src (api 6,764, store 6,369), frontend 159 TS/TSX (~31,903), migrations 52 (~27,478), total tracked ≈ 116,450 LOC

### 2026-08-05 — Session 104: Professional codebase reindex (post-pull refresh)

- Full automated audit: `go vet` clean (13/13 packages incl. sandbox), 9/9 Go test suites green — **146 backend tests** (+10: 6 api format tests, 4 parser parentSummary regression) + **11 sandbox tests** (4 black-gated), zero failures; `go build ./cmd/server` + sandbox OK; ESLint 0 errors, `tsc --noEmit` 0 errors
- `git pull` fast-forwarded `9a29850..745f1d7` (16 files, +531/−180) — Session 103's combined commit (password-recovery overhaul + parser summary-line fix); the CLAUDE.md/SESSION_LOG.md/README.md additions came along with it
- Verified counts: backend **64 src / 15 test files, 18,404 + 3,457 LOC**; sandbox **8 src / 2 test, 1,382 + 228 LOC** (+153: Session 100b `format.go` detailed-error overhaul); frontend **160 TS/TSX files, 32,512 LOC** (73 app 17,816, 65 components 10,887 incl. new `CodeEditor.tsx` 142, `ui/AnimatedBackground.tsx` 73, `ui/ShapeGrid.tsx` 417, 19 lib 3,435, 4 hooks 374); migrations 51 (~27,470); docs 18 (~9,922); scripts 7 (~904); root data 26 (~11,823)
- Updated CLAUDE.md inventory: api 7,183 (admin.go 941, pin_reset.go 296, router.go 303), executor 1,940 (parser.go 132, executor.go 1,306, sandbox_client.go 239), store 6,400 (users.go 1,373), sandbox §7 (main.go 400, format.go 213, format_test.go 196, Dockerfile 38), §8.1/8.2/8.4 counts, §15 test table (17 files, 157 tests), §17 metrics (~117,500 total tracked LOC), header/footer verified badge
- Working tree clean on `update`; no source changes — documentation-only reindex
...
- **Dashboard (`home/page.tsx`)** + **problems listing**: now `max-w-7xl mx-auto pt-4 pb-8 px-4 sm:px-6 lg:px-8` so content edges align with the nav's `max-w-7xl` container — kills the double top gap (was `main pt-6` + page `py-6` = 48px) and the redundant side margins; `pt-4` gives a small breathing room under the sticky nav
- **Settings + Admin**: `settings` root `py-8` → `pt-4 pb-8`; `admin` root `pb-12` → `pt-4 pb-12` — consistent top margin (keeps `max-w-6xl` + px)
...
- **`sandbox/format.go`** — failed Python formats now return a professional, actionable message instead of black's raw `error: cannot format -: Cannot parse: 7:0: ...` (black reports positions against its _normalized_ source, so line/col were misleading, e.g. `7:0` for a 1-line file)
